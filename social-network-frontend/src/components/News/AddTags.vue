<template>
  <div class="add-tags" ref="searchBoxRef">
    <div class="add-tags__form">
      ...
      <div v-if="itemTag.length > 0" class="add-tags__btn" @click="addTag">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const tagsList = ref([]); // Список добавленных тегов
    const itemTag = ref(""); // Поле для ввода нового тега
    const searchResults = ref([]); // Результаты поиска тегов
...
const deleteTag = (index) => {
    ...
    const addTagFromSearch = (tag) => {
      // Добавляем тег из результатов поиска
      ...
      tagsList.value.push(tag); // Добавляем тег в tagsList
      itemTag.value = ""; // Очищаем поле ввода
      searchResults.value = []; // Очищаем результаты поиска
      emit("change-tags", tagsList.value); // Передаем обновленный список тегов
      nextTick(() => {
    ...
    const addTag = () => {
      ...
      tagsList.value.push(newTag); // Добавляем новый тег
      itemTag.value = ""; // Очищаем поле ввода
      emit("change-tags", tagsList.value); // Передаем обновленный список тегов
      nextTick(() => {
        searchInputRef.value.focus(); // Фокусируемся на поле ввода
      });
    };
