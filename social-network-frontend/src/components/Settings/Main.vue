  ...
  setup() {
    ...
    const submitHandler = async () => {
      ...
      if (fileName.value) {
        const imagePayload = {
            file: fileName.value,
            oldImage: getInfo.value.photo,
        }
        await dispatch("global/storage/apiStorage", imagePayload).then(
          (response) => {
      ...
      if (delImage.value) {
        axios.get("/storage/deleteByLink", { params : { linkToDelete: delImage.value }});
      }

      if (profileCover.value) {
        const imagePayload = {
          file: profileCover.value,
          oldImage: getInfo.value.profileCover,
        }
        await dispatch("global/storage/apiStorage", imagePayload).then(
          (response) => {
      ...
      if (delCoverImage.value) {
        axios.get("/storage/deleteByLink", { params : { linkToDelete: delCoverImage.value }});
      }
    ...
    const deletePhoto = () => {
      delImage.value = getInfo.value.photo;
      fileName.value = "";
      src.value = "";
    ...
    const deletePhotoCover = () => {
      delCoverImage.value = getInfo.value.profileCover;
      profileCover.value = "";
      srcCover.value = "";
      commit("global/storage/setStorage", "");
    };
