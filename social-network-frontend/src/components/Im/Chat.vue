...
export default {
  ...
  setup(props) {
    ...
    onMounted(async () => {
      const topicId = props.info.conversationPartner1+props.info.conversationPartner2;

      // Подключаемся к WebSocket
      await $socket.connect();

      // Подписка на топик, связанный с конкретным диалогом
      const topic = `/topic/${topicId}`;
      $socket.subscribe(topic, (message) => {
        console.log("Received message:", message);
        newMessage(message);
        if (!messageDialog.value.some((msg) => msg.id === message.id)) {
          messageDialog.value.push(message);
        }
      });
    ...
    const onSubmitMessage = () => {
      ...
      $socket.sendMessage(payload, topic);
      lastId.value -= 1;
      mes.value = "";
    };
